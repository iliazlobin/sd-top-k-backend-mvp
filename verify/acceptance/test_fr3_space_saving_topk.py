...
def test_space_saving_top_item_ranks_first(client):
    ...
    # 1000 events for "hot"
    for i in range(1000):
        events.append({"item_id": "hot", "event_type": "view",
    ...
    for j in range(1, 10):
        item = f"warm_{j}"
        for i in range(100):
            events.append({"item_id": item, "event_type": "view",
    ...
    assert items[0]["count"] >= 1000, \
        f"Expected 'hot' count ≥ 1000, got {items[0]['count']}"
...
def test_space_saving_result_count_le_k(client):
    ...
    events = []
    for i in range(5):
        events.append({"item_id": f"item_{i}", "event_type": "view",
                       "timestamp": 1719876543000 + i * 100})
        events.append({"item_id": f"item_{i}", "event_type": "view",
                       "timestamp": 1719876543000 + i * 100 + 1})
