...
def test_ingest_single_event_success(client):
    ...
    body = assert_202(
        client.post(
            "/events",
            json={"events": [{"item_id": "a", "event_type": "view", "timestamp": 1719876543000}]},
        )
    )
...
def test_ingest_missing_item_id_422(client):
    ...
    assert_422(
        client.post(
            "/events", json={"events": [{"event_type": "view", "timestamp": 1719876543000}]}
        )
    )
...
def test_ingest_missing_timestamp_422(client):
    ...
    assert_422(client.post("/events", json={"events": [{"item_id": "a", "event_type": "view"}]}))
...
def test_ingest_timestamp_zero_or_negative_422(client):
    ...
    assert_422(
        client.post(
            "/events", json={"events": [{"item_id": "a", "event_type": "view", "timestamp": 0}]}
        )
    )
    assert_422(
        client.post(
            "/events", json={"events": [{"item_id": "a", "event_type": "view", "timestamp": -1}]}
        )
    )
...
def test_ingest_batch_too_large_422(client, batch_size):
    ...
    events = [
        {"item_id": f"overflow_{i}", "event_type": "view", "timestamp": 1719876543000 + i}
