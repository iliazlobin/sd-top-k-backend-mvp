...
def test_cms_count_known_item(client):
    ...
    events = [{"item_id": "x", "event_type": "view",
               "timestamp": 1719876543000 + i} for i in range(100)]
...
def test_cms_count_grows_with_more_events(client):
    ...
    assert_202(client.post("/events", json={
        "events": [{"item_id": "monotonic", "event_type": "click",
                     "timestamp": 1719876543000 + i} for i in range(50)]
    }))
    count1 = assert_json_200(client.get("/count", params={
        "item_id": "monotonic", "window": "1h"
    }))["count"]

    # Second batch
    assert_202(client.post("/events", json={
        "events": [{"item_id": "monotonic", "event_type": "click",
                     "timestamp": 1719876543000 + i} for i in range(50, 100)]
    }))
    count2 = assert_json_200(client.get("/count", params={
        "item_id": "monotonic", "window": "1h"
    }))["count"]
