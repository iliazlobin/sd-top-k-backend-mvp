...
def test_dedup_same_user_item_minute(client):
    ...
    count_resp = assert_json_200(
        client.get("/count", params={"item_id": "dedup_test", "window": "1h"})
    )
...
def test_dedup_same_user_different_items_counted(client):
    ...
    c1 = assert_json_200(client.get("/count", params={"item_id": "item_1", "window": "1h"}))
    c2 = assert_json_200(client.get("/count", params={"item_id": "item_2", "window": "1h"}))
...
def test_dedup_without_user_id_no_dedup(client):
    ...
    assert_202(
        client.post(
            "/events",
            json={"events": [{"item_id": "no_user", "event_type": "click", "timestamp": t}]},
        )
    )
    assert_202(
        client.post(
            "/events",
            json={"events": [{"item_id": "no_user", "event_type": "click", "timestamp": t + 1}]},
        )
    )
